<!-- app/Views/admin/questionnaire/_breadcrumb.php -->
<nav aria-label="breadcrumb" class="mb-3 p-2 bg-white">
    <ol class="breadcrumb-custom">
        ...
        $is_home    = uri_string() === 'admin/questionnaire';
        ?>

        <!-- Homepage -->
        <li class="<?= $is_home ? 'active' : '' ?>">
            <a href="<?= base_url('admin/questionnaire') ?>">Homepage</a>
        </li>

        <!-- Pages -->
        <?php if ($q_id): ?>
            <li class="<?= ($segments[3] ?? null) === 'pages' && !$page_id ? 'active' : '' ?>">
                <a href="<?= base_url("admin/questionnaire/$q_id/pages") ?>">Pages</a>
            </li>
        <?php endif; ?>
        ...
        <?php if ($page_id): ?>
            <li class="<?= ($segments[5] ?? null) === 'sections' && !$section_id ? 'active' : '' ?>">
                <a href="<?= base_url("admin/questionnaire/$q_id/pages/$page_id/sections") ?>">Section</a>
            </li>
        <?php endif; ?>
        ...
        <?php if ($section_id && ($segments[7] ?? null) === 'questions'): ?>
            <li class="active">
                <a href="<?= base_url("admin/questionnaire/$q_id/pages/$page_id/sections/$section_id/questions") ?>">Question</a>
            </li>
        <?php endif; ?>
    </ol>
</nav>
